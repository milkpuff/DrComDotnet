...
def log(*args, **kwargs):
    print(*args, **kwargs)  #换掉原来的join，清真一些
    if DEBUG:
        pass
...
def challenge(svr, ran):
    while True:
        ...
        if address == (svr, 61440):
            ...
            exit()
    log('[DEBUG] challenge:\n' + data.hex())
    if data[0] != 2:                             #date[0] 变成了int型
        log(f"错误,data[0] 是 {data[0]}不是 \\x02")
...
def mkpkt(salt, usr, pwd, mac):
    ...
    data += b''.join([int(x).to_bytes(1,'big') for x in host_ip.split('.')])    # to_bytes更清真
    ...
    data += dump(mac)
    if (len(pwd) / 4) != 4:
        data += b'\x00' * (len(pwd) // 4)  # strange。。。  # 地板除
